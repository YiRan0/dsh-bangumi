 ...
 */
import { parseEpisode } from './parse.js';
    ...
    while ((m = itemRe.exec(xml)) !== null) {
        ...
        if (magnet && !magnet.startsWith('magnet:')) {
            if (link?.startsWith('magnet:'))
                magnet = link;
            else if (guid?.startsWith('magnet:'))
                magnet = guid;
        }
        const item = {
            title,
...
/** nyaa 搜索 RSS */
export function nyaaSearchUrl(query) {
    return 'https://nyaa.si/?page=rss&f=0&c=1_4&q=' + encodeURIComponent(query);
}
/** 动漫花园搜索 RSS */
export function dmhySearchUrl(query) {
...
export async function searchDmhy(query) {
