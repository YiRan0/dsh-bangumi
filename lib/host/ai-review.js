...
export class AiReviewer {
    ...
    async reviewCandidates(input) {
        const list = input.candidates.map((c, i) => '#' + (i + 1) + ' ' + (c.full ? '[全集' + (c.range ?? '') + ']' : '[EP' + c.episode + ']') + ' score=' + c.score + ' ' + (c.group ? c.group + ' ' : '') + (c.pubDate ? c.pubDate + ' ' : '') + c.title).join('\n');
        const system = [
        ...
        const num = (v) => Array.isArray(v) ? v.filter((x) => typeof x === 'number') : [];
        return {
            decision,
