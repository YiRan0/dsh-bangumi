...
class BangumiDb {
    ...
    constructor(path) {
        ...
        for (const s of [SUB_SCHEMA, DL_SCHEMA, LIB_SCHEMA, CACHE_SCHEMA])
            this.db.exec(s);
        // 已有库（旧 schema 无游标列）就地补列
        for (const c of ['last_check_at', 'last_episode_at', 'status']) {
            try {
                this.db.exec('ALTER TABLE subscriptions ADD COLUMN ' + c + ' ' + (c === 'last_check_at' ? 'INTEGER NOT NULL DEFAULT 0' : c === 'status' ? 'TEXT NOT NULL DEFAULT \'unknown\'' : 'INTEGER'));
            }
            catch { /* 已存在 */ }
        }
        try {
            this.db.exec("ALTER TABLE downloads ADD COLUMN full INTEGER");
        }
        catch { /* 已存在 */ }
        try {
            this.db.exec("ALTER TABLE downloads ADD COLUMN range_from INTEGER");
        }
        catch { /* 已存在 */ }
        try {
            this.db.exec("ALTER TABLE downloads ADD COLUMN range_to INTEGER");
        }
        catch { /* 已存在 */ }
        try {
            this.db.exec('ALTER TABLE subscriptions ADD COLUMN completed_at INTEGER');
        }
        catch { /* 已存在 */ }
        try {
            this.db.exec('ALTER TABLE subscriptions ADD COLUMN season INTEGER');
        }
        catch { /* 已存在 */ }
    }
    // ---------- 订阅 ----------
    listSubscriptions() {
        const rows = this.db.prepare('SELECT * FROM subscriptions ORDER BY created_at').all();
    ...
    listDownloads() {
        const rows = this.db.prepare('SELECT * FROM downloads ORDER BY at DESC').all();
        return rows.map((r) => ({
    ...
    replaceLibrary(files, roots, scannedAt) {
        this.db.exec('BEGIN');
        try {
    ...
    loadLibraryFiles() {
        const rows = this.db.prepare('SELECT * FROM library_files').all();
        return rows.map((r) => ({
    ...
    getLibraryMeta() {
        ...
        let roots = [];
        try {
            roots = JSON.parse(this.getMeta('roots') ?? '[]');
        }
        catch { /* 默认空 */ }
    ...
    // ---------- bgm 元数据缓存 ----------
    getSubjectCache(id) {
        const row = this.db.prepare('SELECT payload, saved_at FROM subject_cache WHERE id = ?').get(id);
        if (!row)
            return null;
        if (Date.now() - Number(row.saved_at) > SUBJECT_TTL_MS)
            return null;
        try {
    ...
    setSubjectCache(id, subject, aliases) {
    ...
    listSubjectCacheAll() {
        const rows = this.db.prepare('SELECT id, payload FROM subject_cache').all();
        const out = [];
        for (const row of rows) {
            try {
                const p = JSON.parse(String(row.payload));
                if (p && typeof p.subject === 'object' && p.subject !== null) {
                    out.push({ id: Number(row.id), subject: p.subject, aliases: Array.isArray(p.aliases) ? p.aliases : [] });
                }
            }
            catch { /* 单条损坏跳过 */ }
        }
    ...
    getEpisodesCache(subjectId, allowStale = false) {
        const row = this.db.prepare('SELECT payload, saved_at FROM episodes_cache WHERE subject_id = ?').get(subjectId);
        if (!row)
            return null;
    ...
    listLogs(limit = 200) {
        return this.db.prepare('SELECT id,ts,level,tag,message FROM activity_log ORDER BY id DESC LIMIT ?').all(limit);
    }
    clearLogs() { try {
        this.db.exec('DELETE FROM activity_log');
    }
    catch { /* ok */ } }
    setEpisodesCache(subjectId, episodes) {
    ...
    getSubjectCover(subjectId, url) {
        const row = this.db.prepare('SELECT url, bytes, content_type FROM cover_cache WHERE subject_id = ?').get(subjectId);
        if (!row || row.url !== url)
            return null;
        return { bytes: row.bytes, contentType: String(row.content_type ?? 'image/jpeg') };
    }
    putSubjectCover(subjectId, url, bytes, contentType) {
    ...
    getMeta(key) {
        const row = this.db.prepare('SELECT value FROM library_meta WHERE key = ?').get(key);
    ...
    get tableStats() {
        ...
        for (const t of ['subscriptions', 'downloads', 'library_files', 'subject_cache', 'episodes_cache']) {
            const row = this.db.prepare(`SELECT count(*) c FROM ${t}`).get();
            out[t] = Number(row.c);
        }
        return out;
    }
...
function migrateLegacy(db) {
    ...
    try {
        renameSync(LEGACY_PATH, LEGACY_PATH + '.bak-v1');
    }
    catch { /* 无碍 */ }
}
let instance = null;
...
export function logActivity(level, tag, message) {
    try {
        getDb().addLog(level, tag, message);
    }
    catch { /* 初始化期可静默 */ }
}
