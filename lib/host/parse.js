...
export function parseEpisode(filename) {
    ...
    if (res)
        out.resolution = res[0].toLowerCase().includes('k') ? res[0].toUpperCase() : res[0].toLowerCase();
    // 季号
    ...
    for (const re of [
        ...
        while ((m = re.exec(name)) !== null) {
            ...
            if (v >= 0 && v <= 999)
                candidates.push({ value: v, index: m.index });
        }
    }
    // 排除明显是年份 / 分辨率 / 音轨的数字
    const filtered = candidates.filter((c) => {
        if ([1080, 720, 480, 2160, 3840].includes(c.value))
            return false;
        if (c.value >= 1990 && c.value <= 2100)
            return false;
        return true;
    });
    ...
    const titleSeg = brackets.find((seg, i) => {
        ...
        if (RES_RE.test(seg))
            return false;
        if (/^\d{1,3}(\.5)?(v\d)?$/i.test(seg.trim()))
            return false;
 ...
 */
export function detectPack(raw) {
    const t = raw;
    const flags = [];
    if (!t)
        return null;
    ...
    if (rm && !multiSeason) {
        ...
        if (!seasonLead) {
            const f = parseFloat(rm[1]);
            const e = parseFloat(rm[2]);
            if (f <= e) {
                from = f;
                to = e;
                rangeRaw = rm[0].trim();
                const after = t.slice(rm.index + rm[0].length, rm.index + rm[0].length + 24);
                if (/SP|特典|OVA|总集编/i.test(after))
                    flags.push('range-with-sp');
                flags.push('range:' + rm[0].trim());
                if (spanCount(from, to) >= 8)
                    flags.push('wide-range:' + spanCount(from, to));
            }
        }
    }
    // 标记词
