...
import { getEpisodesCached, getSubjectCached, searchSubjects } from './host/bangumi.js';
import { configureNet, getNetConfig, netFetch } from './host/net.js';
import { subjectCardHtml } from './host/card.js';
import { scanLibrary, buildByTitle, buildByDir, findEpisodesInLibrary, findBestCandidate } from './host/library.js';
import { getDb, logActivity } from './host/db.js';
import { rankItems } from './host/match.js';
import { normalizeTitle, parseEpisode } from './host/parse.js';
import { decideDownloads } from './host/decision.js';
import { QbClient } from './host/qb.js';
import { dmhySearchUrl, fetchFeed, nyaaSearchUrl, searchDmhy, searchNyaa } from './host/rss.js';
import { getState, saveState } from './host/store.js';
import { AiReviewer } from './host/ai-review.js';
const DBG = homedir() + '/.dsh/dsh-bangumi-debug.log';
function dbg(msg) {
    try {
        const st = statSync(DBG, { throwIfNoEntry: false });
        if (st && st.size > 512 * 1024)
            truncateSync(DBG, Math.floor(st.size / 2)); // 超 512KB 截半防膨胀
...
function loadFileConfig() {
    try {
        ...
        const out = {};
        for (const k of ['qbUrl', 'qbUsername', 'qbPassword', 'qbSavePath', 'qbCategory', 'qbTags', 'qbCategoryRoot', 'qbBaseDir']) {
            if (typeof obj[k] === 'string')
                out[k] = obj[k];
        }
        if (Array.isArray(obj.mediaDirs))
            out.mediaDirs = obj.mediaDirs.filter((s) => typeof s === 'string');
        if (typeof obj.pollIntervalMinutes === 'number' && obj.pollIntervalMinutes > 0)
            out.pollIntervalMinutes = obj.pollIntervalMinutes;
        if (typeof obj.minMatchScore === 'number')
            out.minMatchScore = obj.minMatchScore;
        if (typeof obj.rssIgnoreDays === 'number' && obj.rssIgnoreDays >= 0)
            out.rssIgnoreDays = obj.rssIgnoreDays;
        if (typeof obj.aiEnabled === 'boolean')
            out.aiEnabled = obj.aiEnabled;
        if (typeof obj.aiProvider === 'string')
            out.aiProvider = obj.aiProvider;
        if (typeof obj.aiModel === 'string')
            out.aiModel = obj.aiModel;
        if (obj.proxyType === 'http' || obj.proxyType === 'https' || obj.proxyType === 'socks5' || obj.proxyType === 'none')
            out.proxyType = obj.proxyType;
        if (typeof obj.proxyHost === 'string')
            out.proxyHost = obj.proxyHost;
        if (typeof obj.proxyPort === 'number')
            out.proxyPort = obj.proxyPort;
        if (typeof obj.proxyUsername === 'string')
            out.proxyUsername = obj.proxyUsername;
        if (typeof obj.proxyPassword === 'string')
            out.proxyPassword = obj.proxyPassword;
        return out;
    }
    catch {
        return {};
...
async function progressFor(rt, sub) {
    ...
    const have = new Set([...lib.episodes, ...qbEps]);
 ...
 */
async function subscribeBangumi(rt, bangumiId, opts, logger) {
    ...
    if (season >= 2) {
        try {
            await migrateFlatFirstSeason(rt, lay.core, logger);
        }
        catch { /* 迁移失败不影响订阅 */ }
    }
    return sub;
...
async function unsubscribeBangumi(rt, id) {
    const state = getState();
 ...
 */
async function alreadyHaveEpisodes(rt, sub) {
    const have = new Set();
    // 1) 本地媒体库
    if (rt.library) {
        const lib = findEpisodesInLibrary(rt.library, sub.aliases);
        for (const n of lib.episodes)
            have.add(n);
    }
...
function pickTorrent(cands, prefer) {
 ...
 */
async function aiApproveDownload(rt, info, logger) {
 ...
 */
async function pollSubscriptions(rt, logger) {
    const state = getState();
    const got = [];
    for (const sub of state.subscriptions) {
        ...
        try {
            ...
            const windowed = ranked.filter((c) => {
                if (c.pack?.isPack)
                    return true; // 整包（全集）始终候选（完结判定在决策层把关）
            ...
            let epsAll = await getEpisodesCached(sub.bangumiId).catch(() => []);
            ...
            const lib = rt.library ? findEpisodesInLibrary(rt.library, sub.aliases) : { episodes: [], count: 0 };
            ...
            dbg('ai-gate sub=' + sub.id + ' enabled=' + rt.ai.enabled + ' ranked=' + ranked.length + ' windowed=' + windowed.length + ' have=' + JSON.stringify([...have].sort((a, b) => a - b)));
            if (rt.ai.enabled) {
                ...
                if (aiDecided && aiDecided.decision === 'download-selected') {
                    ...
                    logger.info('bangumi ai-select: sub=%s select [%s] of %s candidates (%s)', sub.id, [...sel].sort((a, b) => a - b).join(','), windowed.length, aiDecided.reason);
                }
            ...
            dbg('ai-done sub=' + sub.id + ' verdict=' + (aiDecided ? aiDecided.decision : 'null') + ' aiCands=' + aiCandidates.length + ' actions=' + dec.actions.map((a) => (a.episode ?? (a.full ? 'full' : '?')).toString()).join(',') + ' why=' + dec.actions.map((a) => a.why).join('|').slice(0, 80));
 ...
 */
async function cleanupLegacyRss(rt, logger) {
    const state = getState();
...
async function searchTorrentsForBangumi(rt, bangumiId, options = {}) {
    const { subject, aliases } = await subjectBundle(bangumiId);
...
function seasonOf(raw) {
    ...
    if (/第?[一二三四五]s*季|第?[二三四五]期|(?:^|[^0-9])2s*期|(?:^|[^0-9])3s*期/.test(n)) {
        for (const [kw, v] of [['第五季', 5], ['第四季', 4], ['第三季', 3], ['第二季', 2], ['第五期', 5], ['第四期', 4], ['第三期', 3], ['第二期', 2], ['二期', 2], ['三期', 3], ['四期', 4], ['五期', 5]]) {
            if (n.includes(kw))
                return v;
        }
    }
    ...
    if (/parts*3/.test(n))
        return 3;
    if (/2nd|second/.test(n))
        return 2;
    if (/3rd|third/.test(n))
        return 3;
    return 1;
}
function hasSeasonWord(raw) {
    const n = normalizeTitle(raw);
...
function layoutForTitle(rt, title) {
    const state = getState();
    ...
    for (const s of state.subscriptions) {
        const cands = [s.nameCn, s.name, ...(s.aliases ?? [])].filter(Boolean);
...
async function ensureCategory(rt, category, savePath, logger) {
    try {
 ...
 */
async function migrateFlatFirstSeason(rt, core, logger) {
    const root = (rt.cfg.qbCategoryRoot || '番').replace(/\/+$/, '');
    const base = (rt.cfg.qbBaseDir || '').replace(/\/+$/, '');
    ...
    if (hasLaterSub) {
        ...
        for (const s of st.subscriptions) {
            if (coreNameOf(s) !== core || (s.season ?? 1) !== 1)
                continue;
    ...
    try {
        ...
        for (const [cat, hashes] of groups) {
            ...
            try {
                ...
                if (base)
                    await rt.qb.setTorrentSavePath(hashes.join('|'), base + '/' + core + '/' + seg);
 ...
 */
async function syncCategoryLayout(rt, logger) {
    const state = getState();
    const base = (rt.cfg.qbBaseDir || '').replace(/\/+$/, '');
        ...
        if (needChange) {
            ...
            try {
                ...
                if (mine.length) {
                    ...
                    if (base && lay.savePath)
                        await rt.qb.setTorrentSavePath(hashes, lay.savePath);
                }
    ...
    if (changedRows) {
        try {
            saveState(state);
            dbg('sync-cat saveState ok');
        }
        catch (err) {
            dbg('sync-cat saveState ERR ' + (err instanceof Error ? err.message : String(err)));
        }
    }
    ...
    for (const s of getState().subscriptions)
        liveCats.add(s.category);
    let removed = 0;
    for (const st of stale) {
        if (!st.category || st.category === rt.cfg.qbCategory || !existing.has(st.category))
            continue;
        if (liveCats.has(st.category))
            continue;
        try {
...
async function lookupSubject(keyword) {
    ...
    if (!subjects.length)
        subjects = await searchSubjects(kw, 12);
    if (!subjects.length)
        return null;
    ...
    if (wantSeasonWord) {
        ...
        if (!seasonal.length)
            return null; // 候选里没有该季（bgm 未建条目）→ 宁缺毋滥
        ...
        const dateVal = (d) => (d ? d.replace(/[^0-9]/g, '') : '');
...
async function llmModelCatalog(bctx, provider) {
    try {
        const llm = bctx.llm;
        ...
        if (llm?.listModels && target) {
            try {
                models = await llm.listModels(target);
            }
            catch {
                models = [];
            }
...
export function apply(ctx, config) {
    ...
    const aiCfg = () => ({ enabled: rt.cfg.aiEnabled, route: rt.cfg.aiProvider && rt.cfg.aiModel ? { provider: rt.cfg.aiProvider, model: rt.cfg.aiModel } : undefined });
    dbg('apply llm=' + (bctx.llm ? 'present' : 'MISSING') + ' ai=' + rt.cfg.aiEnabled + ' ' + rt.cfg.aiProvider + '/' + rt.cfg.aiModel);
    rt.ai = new AiReviewer(bctx.llm ?? null, aiCfg, (m) => { logger.warn('%s', m); dbg('ai-log ' + m); });
    const reloadRuntime = () => {
    ...
    const getLibrary = async () => {
        ...
        if (rt.cfg.mediaDirs.length === 0)
            return { files: [], byTitle: {}, byDir: {}, scannedAt: meta.scannedAt, roots: [] };
        const needRescan = !meta.scannedAt ||
            meta.roots.length === 0 ||
    ...
    const doLibraryScan = async () => {
        if (rt.libraryScanning)
            return rt.library ?? { files: [], byTitle: {}, byDir: {}, scannedAt: 0, roots: [] };
            ...
            if (rt.ai.enabled) {
                try {
                    ...
                    for (const [key, eps] of Object.entries(snap.byDir)) {
                        ...
                        if (added.length)
                            changedWorks.push({ name: key, episodes: added.sort((a, b) => a - b) });
                    }
                    // 订阅已放送缺集（只为有变化的作品匹配的订阅拉 episodes，控制成本）
                    const st0 = getState();
                    const subscribedMissing = [];
                    if (changedWorks.length) {
                        ...
                        for (const sub of st0.subscriptions) {
                            ...
                            const epsAll0 = await getEpisodesCached(sub.bangumiId).catch(() => []);
                            ...
                            if (missing.length)
                                subscribedMissing.push({ name: sub.nameCn || sub.name, missing });
                        }
                    }
                    ...
                    if (note) {
                        ...
                        if (note.notifyMissing) {
                            ...
                            void pollSubscriptions(rt, logger).then((fr) => {
                                if (fr.length)
                                    logger.info('bangumi ai-triggered poll downloaded %d', fr.length);
                ...
                try {
                    ...
                    if (route === '/subscriptions' && req.method === 'POST') {
                        ...
                        void pollSubscriptions(rt, logger).then((fresh) => {
                            if (fresh.length)
                                logger.info('bangumi post-subscribe fetched %d episode(s) for %s', fresh.length, sub.id);
                    ...
                    if (route === '/search' && req.method === 'GET') {
                        ...
                        const lib = libSnap ? findEpisodesInLibrary(libSnap, out.subject.aliases ?? []) : { episodes: [], count: 0 };
                        return sendJson(res, 200, {
                            subject: out.subject,
                        ...
                        try {
                            ...
                            if (preflight.status === 404) {
                                ...
                                try {
                                    ...
                                    if (fresh && effSid) {
                                        try {
                                            getDb().setSubjectCache(effSid, fresh, fresh.aliases ?? []);
                                        }
                                        catch { /* noop */ }
                                    }
                                }
                            ...
                            const up = preflight;
                            if (!up.ok)
                                throw new Error('upstream ' + up.status + ' ' + effTarget.slice(-60));
                    ...
                    if (route === '/download' && req.method === 'POST') {
                        ...
                        if (subRow) {
                            ...
                            cat = lay.category;
                            sp = lay.savePath || rt.cfg.qbSavePath;
                        }
                        ...
                        if (cat !== rt.cfg.qbCategory)
                            await ensureCategory(rt, cat, sp, logger);
                        ...
                        if (bgmId || dtitle) {
                            ...
                            if (!gated.ok)
                                return sendJson(res, 403, { ok: false, error: 'AI 审核未通过（详情见日志）' });
                        }
                    ...
                    if (route === '/library/posters' && req.method === 'GET') {
                        ...
                        for (const f of files) {
                            ...
                            let g = works.get(key);
                            if (!g) {
                                g = { key, dir: f.dir, episodes: [], names: [] };
                                works.set(key, g);
                            }
                            g.episodes.push(f.parsed.episode);
                            if (g.names.length < 3 && !g.names.includes(f.name))
                                g.names.push(f.name);
                        }
                        ...
                        for (const g of works.values()) {
                            ...
                            for (const k of matchKeys) {
                                best = findBestCandidate(k, subCands);
                                if (best)
                                    break;
                            }
                            if (!best)
                                for (const k of matchKeys) {
                                    best = findBestCandidate(k, cands);
                                    if (best)
                                        break;
                                }
                            if (!best) {
                        ...
                        await Promise.all(posters.map(async (p) => {
                            ...
                            try {
                                ...
                                if (dated.length)
                                    p.aired = dated.length;
                                // totalEpisodes 缺失/为 0 时用 episodes 表条数兜底（bgm 搜索响应常标 0）
                                if (p.totalEpisodes === undefined || p.totalEpisodes <= 0)
                                    p.totalEpisodes = eps.length;
                            }
                    ...
                    if (route === '/calendar' && req.method === 'GET') {
                        ...
                        for (const sub of state.subscriptions) {
                            let map = new Map();
                            try {
                                const eps = await getEpisodesCached(sub.bangumiId).catch(() => []);
                                for (const e of eps)
                                    if (e.airDate)
                                        map.set(e.airDate, e.ep);
                            }
                            catch { /* keep empty */ }
                            ...
                            for (const { sub, map } of perSubEps) {
                                ...
                                if (known !== undefined) {
                                    ...
                                    if (sub.weekday !== undefined && date.getDay() === sub.weekday && sub.airDate) {
                                        ...
                                        if (est !== null && (sub.totalEpisodes === undefined || est <= sub.totalEpisodes))
                                            ep = est;
                                    }
                                }
                                if (ep !== null)
                                    items.push({ name: sub.name, nameCn: sub.nameCn, episode: ep, source: sub.source });
                            }
                            if (items.length)
                                calendar.push({ date: iso, items });
                        }
                    ...
                    if (route === '/lookup' && req.method === 'GET') {
                        ...
                        if (!subject)
                            return sendJson(res, 404, { error: 'no unique subject for: ' + kw });
                        const bundle = await subjectBundle(subject.id);
                        const libSnap = await getLibrary();
                        const lib = libSnap ? findEpisodesInLibrary(libSnap, bundle.aliases) : { episodes: [], count: 0 };
    ...
    ctx.effect(() => [
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args) {
                ...
                if (!subs.length)
                    return '未找到番剧: ' + args.keyword;
                const nl = String.fromCharCode(10);
                const lines = ['找到 ' + subs.length + ' 部番剧：'];
                for (const s of subs) {
        ...
        ctx.tools.register(defineTool({
            ...
            async execute() {
                ...
                if (!state.subscriptions.length)
                    return '暂无订阅。用 bangumi_lookup + bangumi_subscribe 开始追番。';
                const nl = String.fromCharCode(10);
                const lines = ['当前订阅 ' + state.subscriptions.length + ' 部：'];
                for (const sub of state.subscriptions) {
        ...
        ctx.tools.register(defineTool({
            ...
            async execute() {
                ...
                const state = getState();
                const nl = String.fromCharCode(10);
                const lines = [now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + ' 追番表：'];
                ...
                for (let d = 1; d <= days; d += 1) {
                    ...
                    const items = [];
                    for (const sub of state.subscriptions) {
                        if (sub.weekday === undefined || date.getDay() !== sub.weekday)
                            continue;
                        let ep = null;
                        if (sub.airDate) {
                            const first = new Date(sub.airDate + 'T00:00:00');
                            if (date >= first)
                                ep = Math.floor((date.getTime() - first.getTime()) / (7 * 86400_000)) + 1;
                        }
                        items.push((sub.nameCn || sub.name) + (ep ? ' EP' + ep : ''));
                    }
                    if (items.length) {
                        any = true;
                        lines.push(String(d).padStart(2, '0') + ': ' + items.join(', '));
                    }
                }
                if (!any)
                    lines.push('（本月暂无订阅更新）');
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args) {
                ...
                if (ex(FILE_CONFIG)) {
                    try {
                        cur = JSON.parse(rf(FILE_CONFIG, 'utf8'));
                    }
                    catch {
                        cur = {};
                    }
                ...
                if (args.url)
                    cur.qbUrl = args.url;
                if (args.username)
                    cur.qbUsername = args.username;
                if (args.password)
                    cur.qbPassword = args.password;
                if (args.savePath)
                    cur.qbSavePath = args.savePath;
                if (args.category)
                    cur.qbCategory = args.category;
                if (args.categoryRoot)
                    cur.qbCategoryRoot = args.categoryRoot;
                if (args.baseDir)
                    cur.qbBaseDir = args.baseDir;
                if (args.tags)
                    cur.qbTags = args.tags;
        ...
        const tick = async () => {
            try {
                ...
                if (!cleanedLegacy) {
                    ...
                    try {
                        await syncCategoryLayout(rt, logger);
                    }
                    catch (err) {
