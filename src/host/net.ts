...
export const NO_PROXY: ProxyConfig = { type: 'none', host: '', port: 0, username: '', password: '' }

export function isProxyEnabled(p: ProxyConfig | null | undefined): boolean {
  return !!p && p.type !== 'none' && !!p.host && p.port > 0
}

/* ---------------- socks5 (RFC 1928) ---------------- */

function socks5Connect(p: ProxyConfig, host: string, port: number, signal: AbortSignal): Promise<Socket> {
  return new Promise((resolve, reject) => {
    const socket = net.connect({ host: p.host, port: p.port })
    const onAbort = () => { socket.destroy(); reject(new Error('aborted')) }
    if (signal.aborted) { socket.destroy(); reject(new Error('aborted')); return }
    signal.addEventListener('abort', onAbort, { once: true })

    let step: 'greet' | 'auth' | 'cmd' = 'greet'
    let buf: Buffer<ArrayBufferLike> = Buffer.alloc(0)
    let failed = false
    const fail = (e: Error) => { if (failed) return; failed = true; signal.removeEventListener('abort', onAbort); socket.destroy(); reject(e) }
    ...
    const onData = (chunk: Buffer) => {
      ...
      try {
        ...
        if (step === 'cmd') {
          ...
          const rep = buf[1]
          if (rep !== 0x00) {
            const reasons: Record<number, string> = { 1: 'general failure', 2: 'not allowed', 3: 'network unreachable', 4: 'host unreachable', 5: 'connection refused', 6: 'ttl expired', 7: 'command not supported', 8: 'address type not supported' }
            throw new Error('socks5 connect failed: ' + (reasons[rep] ?? 'code ' + rep))
          }
          const atyp = buf[3]
          let addrStart = 4
          if (atyp === 0x01) addrStart = 8
          else if (atyp === 0x03) { if (buf.length < 5) return; addrStart = 5 + buf[4] }
          else if (atyp === 0x04) addrStart = 20
          else throw new Error('socks5 bad atyp ' + atyp)
          if (buf.length < addrStart + 2) return
          socket.removeAllListeners('data')
          socket.removeAllListeners('error')
          signal.removeEventListener('abort', onAbort)
          socket.on('error', () => { /* 上层 createConnection 已接管后续错误 */ })
          resolve(socket)
        }
      } catch (e) {
...
function httpProxyConnect(p: ProxyConfig, host: string, port: number, signal: AbortSignal): Promise<Socket> {
  return new Promise((resolve, reject) => {
...
function collectBody(res: import('node:http').IncomingMessage): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const chunks: Buffer[] = []
...
function wrapIncoming(res: import('node:http').IncomingMessage): NetResponse {
  const headers: Record<string, string> = {}
  for (let i = 0; i < res.rawHeaders.length; i += 2) headers[res.rawHeaders[i].toLowerCase()] = res.rawHeaders[i + 1]
  ...
  return {
    ...
    buffer,
    arrayBuffer: async () => (await buffer()).buffer.slice(0) as ArrayBuffer,
  }
}

function wrapNative(res: Response): NetResponse {
  const headers: Record<string, string> = {}
...
let current: ProxyConfig = NO_PROXY

/** 设置当前出口代理（设置页保存 / 初始化时调用；none 恢复直连） */
export function configureNet(p: ProxyConfig): void {
  current = {
...
export function getNetConfig(): ProxyConfig { return current }
...
function needsTunnel(method: string): boolean {
 ...
 */
export async function netFetch(url: string, init: NetRequestInit = {}, proxyOverride?: ProxyConfig): Promise<NetResponse> {
  const proxy = proxyOverride ?? current
  const target = new URL(url)
  const method = (init.method ?? 'GET').toUpperCase()
  const headers: Record<string, string> = { ...(init.headers ?? {}) }
  const body: string | Buffer | null = init.body ?? null
...
function tunneledRequest(
  ...
  return new Promise((resolve, reject) => {
    const headersFinal: Record<string, string> = { Host: target.host, ...headers }
    const path = (target.pathname || '/') + target.search
    const onAbort = () => { (sock as Socket).destroy(); reject(new Error('aborted')) }
    const run = (sock: Socket) => {
      signal.addEventListener('abort', onAbort, { once: true })
...
/** 拉文本（非 2xx 抛错） */
export async function netFetchText(url: string, init?: NetRequestInit): Promise<string> {
